...
func buildPrompt(userPrompt string, predictableKeys []KeyDef) string {
	...
	b.WriteString("INLINE ATTRIBUTES: Preserve inline tag attributes verbatim when the DCD equivalent tag supports them. Do NOT strip, rename, or flatten an inline attribute into a [style] block or a paragraph attribute when the inline tag accepts it. Only fall back when DCD has no inline equivalent.\n\n")
	b.WriteString("PRESERVE FONT-FAMILY: Never miss a font-family declaration that exists in the source — carry it over where the source declares it. Never invent a font-family declaration where the source has none, and never wrap individual words in <span> just to carry it.\n")
	b.WriteString("PRESERVE <span> AND <tab>: Every <span> (with all its attributes) and every <tab> tag that EXISTS in the source MUST appear in the output — never drop or flatten them, and never merge spans that carry different formatting. Conversely, NEVER invent <span> tags: wrap exactly the same text range the source wraps — never individual words — and never add a span where the source has none.\n")
