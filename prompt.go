...
func buildPrompt(userPrompt string, predictableKeys []KeyDef) string {
	...
	b.WriteString("PRESERVE FONT-FAMILY: Never miss a font-family declaration that exists in the source — carry it over where the source declares it. Never invent one where the source has none.\n")
	b.WriteString("USE <span> ONLY IF NEEDED: Emit a <span> only when a run's formatting differs from the paragraph default and no dedicated inline tag (<b>/<i>/<u>/<s>/<mark>) applies. Never wrap single words or spaces; never exceed the span count of the source words-xml. Every <tab> tag that EXISTS in the source MUST appear in the output.\n")
